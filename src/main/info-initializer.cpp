...
static errr init_info(player_type* player_ptr, concptr filename, angband_header* head, void** info, char** name, char** text, char** tag) {
    ...
    if (fd >= 0) {
        ...
        fd_write(fd, (concptr)head->info_ptr, head->info_size);
        fd_write(fd, (concptr)head->name_ptr, head->name_size);
        fd_write(fd, (concptr)head->text_ptr, head->text_size);
        fd_write(fd, (concptr)head->tag_ptr, head->tag_size);
        (void)fd_close(fd);
    }
