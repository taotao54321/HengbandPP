...
static void Initialize(AngbandWidget request, AngbandWidget new) {
    ...
    for (n = 0; n < NUM_COLORS; n++) {
        ...
        new->angband.gc[n] = XtGetGC((Widget) new, GCFont | GCForeground | GCBackground | GCGraphicsExposures,
            &gcv);
    }
...
static Boolean SetValues(AngbandWidget current, AngbandWidget request,
    ...
    if (font_changed || border_changed) {
        /* Change window size */
        height = (current->core.height - 2 * current->angband.internal_border) / current->angband.fontheight * new->angband.fontheight + 2 * current->angband.internal_border;
        width = (current->core.width - 2 * current->angband.internal_border) / current->angband.fontwidth * new->angband.fontwidth + 2 * new->angband.internal_border;
