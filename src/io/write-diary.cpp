...
errr exe_write_diary(player_type* creature_ptr, int type, int num, concptr note) {
    ...
    if (type == DIARY_FIX_QUEST_C || type == DIARY_FIX_QUEST_F || type == DIARY_RAND_QUEST_C || type == DIARY_RAND_QUEST_F || type == DIARY_TO_QUEST) {
    ...
    case DIARY_STAIR: {
        concptr to = q_idx && (is_fixed_quest_idx(q_idx) && !((q_idx == QUEST_OBERON) || (q_idx == QUEST_SERPENT)))
            ? _("地上", "the surface")
