...
HIT_POINT spell_RF6_SPECIAL_B(player_type *target_ptr, POSITION y, POSITION x, MONSTER_IDX m_idx, MONSTER_IDX t_idx, int TARGET_TYPE)
{
    ...
    if (monster_to_player || t_idx == target_ptr->riding)
        teleport_player_to(target_ptr, m_ptr->fy, m_ptr->fx, teleport_flags(TELEPORT_NONMAGICAL | TELEPORT_PASSIVE));
    else
        teleport_monster_to(target_ptr, t_idx, m_ptr->fy, m_ptr->fx, 100, teleport_flags(TELEPORT_NONMAGICAL | TELEPORT_PASSIVE));
