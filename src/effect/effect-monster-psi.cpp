...
static bool effect_monster_psi_weird_mind(effect_monster_type* em_ptr) {
    bool has_resistance = ((em_ptr->r_ptr->flags2 & (RF2_STUPID | RF2_WEIRD_MIND)) != 0) || ((em_ptr->r_ptr->flags3 & RF3_ANIMAL) != 0) || (em_ptr->r_ptr->level > randint1(3 * em_ptr->dam));
...
static bool effect_monster_psi_corrupted(player_type* caster_ptr, effect_monster_type* em_ptr) {
    bool is_powerful = ((em_ptr->r_ptr->flags3 & (RF3_UNDEAD | RF3_DEMON)) != 0) && (em_ptr->r_ptr->level > caster_ptr->lev / 2) && one_in_(2);
...
process_result effect_monster_psi_drain(player_type* caster_ptr, effect_monster_type* em_ptr) {
    ...
    else if ((em_ptr->r_ptr->flags2 & (RF2_STUPID | RF2_WEIRD_MIND)) || (em_ptr->r_ptr->flags3 & RF3_ANIMAL) || (em_ptr->r_ptr->level > randint1(3 * em_ptr->dam))) {
...
process_result effect_monster_telekinesis(player_type* caster_ptr, effect_monster_type* em_ptr) {
    ...
    if ((em_ptr->r_ptr->flags1 & RF1_UNIQUE) || (em_ptr->r_ptr->level > 5 + randint1(em_ptr->dam))) {
        em_ptr->do_stun = 0;
